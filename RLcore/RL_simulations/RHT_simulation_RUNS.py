...
def approximated_simulation_runs(
    ...
    """Several Radiative Heat Transfer (RHT) RL approximated method simulations.

    Obtain their promediated results, such as:
    ...
    if perc_runs_top_HTC:
        ...
        for _ in range(n_perc_outputs):
            # crop explored HTCs to plot_range, else get all explored HTCs
            # during each simulation
            range_slice = slice(plot_range) if crop_to_plot_range else slice(None)
            ...
            print(
                f"""\n{perc_best}% of runs explored best HTC, with a value of
                 {best_HTC} W/m²K."""
            )
            ...
            # crop_to_plot_range flag to false
            if crop_to_plot_range:
                print(f"Taken limit of {plot_range} explored states. ")
            crop_to_plot_range = False

        print(
            f"""\nMax HTC values considered above with {len(env.htc_values)} of
             {n_materials**n_layers} states.\n"""
        )
        ...
        if exists(reward_curve_steps_per_point):
            for reward_curve_mode_ in reward_curve_mode:
                ...
                mean_reward_curve.plot(
                    ...
                    save_path=f"./img/several_RUNS/{cfg_save_folder}/Reward_learning_curve_{n_layers}_{suffix}.png",
                )
...
def dummy_simulation_runs(
    ...
    """Obtain naively the "optimal state" obtained with Reinforcement Learning.

    Intended for the Radiative Heat Transfer naive simulation, this is, compute
    the Heat-Transfer Coefficient (HTC) of all states and select the state which
    ...
    if perc_runs_top_HTC:
        ...
        print(
            f"""{perc_top5}% of runs explored top 5 best HTC. \nBest values
             provided with {len(htc_values)} of {n_materials**n_layers}
             states.\n"""
        )
...
def approximated_nd_dummy_simulation_runs(
    ...
    """Generate approximated and dummy simulations to compare them in a shared plot.
    ...
    * img/several_RUNS/apprx_nd_dummy/RHT_{algorithm}
    ...
    See Also
    --------
    Unexplained input parameters are explained in their corresponding function.
