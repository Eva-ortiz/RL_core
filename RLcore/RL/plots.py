...
class learning_curve:
    ...
    def plot(
        ...
        if plot_epsilon and self.epsilon.shape[0] == 0:
            plot_epsilon = False
            warnings.warn(
                "Epsilon curve selected but no epsilon data found. Skipped.",
                stacklevel=1,
            )
        if plot_lr and self.learning_rate.shape[0] == 0:
            plot_lr = False
            warnings.warn(
                """Learning rate curve selected but no learning rate data found.
                Skipped.""",
                stacklevel=1,
            )
...
class mean_learning_curve(learning_curve):
    ...
    def __init__(
        ...
        Warnings
        --------
        We suppose the `learning_rate` and `epsilon` attributes have the same
        ...
        if not np.array_equal(
            ...
            for learn_curve in learning_curves:
                ...
                if curve_diff == 0:
                    ...
                    # DISCLAIMER: we suppose the learning_rate and epsilon
            ...
            # DISCLAIMER: we suppose the learning_rate and epsilon parameters
    ...
    def update(self, **args):
        """Update method for completitude."""
        warnings.warn(
    ...
    def plot(
        ...
        **kwargs,
        ...
        y_divisor = kwargs.get("y_divisor")
        y_divisor = 1 if y_divisor is None else y_divisor
...
class merge_learning_curves_figs(learning_curve):
    ...
    def __init__(
        ...
        curve_fill: Literal["last"] | Number | None = None,
    ):
        ...
        Warnings
        --------
        We suppose `epsilon` attributes have the same decaying rates for all
        ...
        # DISCLAIMER: we suppose epsilon parameter have the same
    ...
    def plot(
        ...
        * mean_learning_curve.plot : `std` input parameter is described in this
        ...
        to plot different algorithm curves, which won't necessarily have the
        ...
        for idx, performance in enumerate(self.curves_performance):
            label = f"learning curve {idx}" if labels is None else labels[idx]
