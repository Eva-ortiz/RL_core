...
class environment(ABC):
    ...
    @abstractmethod
    def step(
        self, state: str | Iterable | Number, action: str | Number
    ) -> tuple[str | Iterable | Number, Number, bool]:
        """Return the next state and reward given an input state and action.

        Additionally, output if the episode is finished if we have an episodic
        task.
    ...
    @abstractmethod
    def reset(self) -> str | Iterable | Number:
...
class agent(ABC):
    ...
    def _act(
        ...
        **kwargs,
        ...
        In other words, the application of the policy.
    ...
    def train(
        ...
        **kwargs :
            Any other of the immense number of parameters to determine for
            training.
        """
        pass
