...
class RHT(environment):
    ...
    * The considered metal is made up, is not real. It is similar to the
    ...
    def __init__(
        ...
        ref_state_label: str | None = None,
        reward_reduction_factor: float | None = None,
        seed: int | None = None,
    ):
        """Initialize of the RHT environment.

        It is componsed by multilayered structures with a vacuum gap between the
        two bodies of study.
    ...
    def reset(
        self,
        init_layers: Literal["initial", "rand"] | str = "initial",
        seed: int | None = None,
    ) -> str:
    ...
    def step(
        ...
        """Return the next state and reward given an input state and action.

        Additionally, output if the episode is finished, defined by if the
        ...
        ** kwargs
            step_count : bool, optional
                ...
                Specially useful for those algorithms that calls this `step`
        ...
        end_episode : bool
            Additional output if we have an episodic task.
        ...
        # manage the length of the episode
        end_episode = False
    ...
    def random_state_label_choice(self):
        """Randomly select an state label."""
        # get a str with a random state
        return (
